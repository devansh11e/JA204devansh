...
export class PrescriptionService {
  ...
  addPrescription(pre:Prescription,token:any):Observable<Prescription>{
    ...
    const patientUrl:string=this.baseUrl+`/api/patients/getPatientbyid/${pre.patientId}`;
    const doctorUrl:string=this.baseUrl+`/api/doctors/getDoctorbyid/${pre.doctorId}`;
    const prescriptionUrl=this.baseUrl+"/api/prescribemedications/addPrescription";
  ...
  { const headers = new HttpHeaders({
    ...
    return this.http.get<Prescription[]>(this.baseUrl+"/api/prescribemedications/getallprescriptions",{headers});}

  getPrescriptionById(prescriptionId:number,token:any):Observable<Prescription>
  {const headers = new HttpHeaders({
    Authorization: 'Bearer ' + token,
  });
    return this.http.get<Prescription>(this.baseUrl+"/api/prescribemedications/getPrescriptionbyid/"+prescriptionId,{headers}).pipe(
    catchError((error) => {
  ...
  getPrescriptionByPatientId(patientId: number,token:any): Observable<Prescription[]> {
    ...
    const url = this.baseUrl+`/api/prescribemedications/getPrescriptionbypatientid/${patientId}`;
